...
const diffPatcher = require('jsondiffpatch').create({ objectHash, arrays });

const History = require('./historyModel').model;

const isValidCb = cb => {
    return cb && typeof cb === 'function';
};

const filterArrayOfString = (arr) => {
    let newArray = [];
    let count = arr.length;

    for (let index = 0; index < count; index++) {
        if (arr[index].includes(".")) {
            newArray.push(arr[index].split(".")[0])
        } else {
...
function saveDiffObject(currentObject, original, updated, opts, queryObject) {
    ...
    //console.log("updated", JSON.parse(JSON.stringify(updated)))
...
/**
 * 
 * @param {Object} queryObject New Object 
 * @param {Object} currentObject Old Object
 * @param {*} opts 
 * @returns 
 */
const saveDiffHistory = (queryObject, currentObject, opts) => {
    ...
    if (strict === true) {
        ...
        const filteredValidPaths = filterArrayOfString(validPaths);
        updatedObject = pick(updatedObject, filteredValidPaths);
    }
    ...
    if (opts.uri) {
        ...
        if (mongoVersion < 5) {
            mongoose.connect(opts.uri, { useMongoClient: true }).catch(e => {
                console.error('mongoose-history connection error:', e);
            });
        } else {
