...
exports.authLogin = catchAsync(async (req, res, next) => {
    ...
    if (!currentUser) {
        return next(
            new AppError(
                "You are attempting to enter an account for which you lack the necessary access permissions.",
                401
            )
        );
    }
...
exports.logout = catchAsync(async (req, res) => {
    ...
    const response = await axios.get('https://www.pctflex.com/server/api/v1/accessControl/logout') // Auth API
    ...
    return async (req, res, next) => {
        ...
        if (!isHaveRole(roles, allUserPermissions)) {
            return next(
                ...
                new AppError(
                    "You dont have the permission for the requested document.",
                    403
                )
            );
        }
